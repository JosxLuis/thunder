<?php
	if(isset($_POST['eliminar-foto']) && $_POST['eliminar-foto'] != ""){
		...
		$borrarEntrada = "UPDATE ".DB_PREFIJO."equipo SET avatar = '' WHERE id".DB_PREFIJO."equipo= ".$_GET['id']."";
	...
	if(isset($_POST['editar']) && $_POST['nombre'] != ""){
		...
		if($_FILES['imagen']['name'] != null){
			...
			if (strlen($error)==0){			
				...
				$avatar = devolverValorQuery("SELECT avatar FROM ".DB_PREFIJO."equipo WHERE id".DB_PREFIJO."equipo=".$_GET['id']." ");
				if($avatar['avatar'] != "" && $avatar['avatar'] != "img/no-picture.jpg"){
		    		unlink("../".$avatar['avatar']);
				}
		
				$ruta_image_normal = $dir."imagen_".$random.".".$file_ext;

				$fotografia = "avatar='".$ruta_image_normal."', ";

			}
		}


		$editarRegistro = "UPDATE ".DB_PREFIJO."equipo SET $fotografia nombre='".utf8_decode($_POST['nombre'])."',apellidos='".utf8_decode($_POST['apellidos'])."',puesto='".utf8_decode($_POST['puesto'])."',facebook='".utf8_decode($_POST['facebook'])."',twitter='".utf8_decode($_POST['twitter'])."',instagram='".utf8_decode($_POST['instagram'])."',correo='".utf8_decode($_POST['correo'])."' WHERE id".DB_PREFIJO."equipo=".$_GET['id']." ";
	...
	$consulta = devolverValorQuery("SELECT *, date_format(fecha_ingreso,'%Y-%m-%d') as fecha_ingreso FROM ".DB_PREFIJO."equipo WHERE id".DB_PREFIJO."equipo=".$_GET['id']." ");
...
<head>
	<meta charset="UTF-8">
	<title>equipos - <?php echo PROYECTO; ?></title>
