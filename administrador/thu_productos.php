...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
    ...
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."producto WHERE id".DB_PREFIJO."producto= ".$_GET['id']."";
    ...
    if(isset($_POST['buscar'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."producto WHERE 1 ".$busqueda."");
        
        if($cantidad != 0){
            $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
            mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
            $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."producto WHERE 1 ".$busqueda." ORDER BY id".DB_PREFIJO."producto
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."producto WHERE 1");
        
        $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."producto  ORDER BY id".DB_PREFIJO."producto DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
				...
				<div class="list-items">
					...
					<div class="left-side-button">
						<form name="busqueda" method="post">
							<input type="text" name="codigo" class="w1" placeholder="Código">
	        			...
	        			<?php if($cantidad != 0){ ?>
	        				<?php 
	        					while($rowProducto = mysqli_fetch_array($resultado)){
	        						//$tipo = devolverValorQuery("SELECT tipo FROM ".DB_PREFIJO."producto_tipo WHERE id".DB_PREFIJO."producto_tipo =".$rowProducto['id'.DB_PREFIJO.'producto_tipo']" ");
	        				?>
			        			<tr>
			        				<td>#<?php echo $rowProducto['id'.DB_PREFIJO.'producto']; ?></td>
			        				<td><div class="foto" <?php if($rowProducto['portada'] != "" ){ ?> style="background:url(<?php echo URL.$rowProducto['portada']; ?>)" <?php } ?>> <?php if($rowProducto['portada'] == "" ){ ?><i class="fa-icon-barcode"><?php } ?></div></td>
			        				<td><?php echo $rowProducto['clave']; ?></td>
			        				<td><?php echo utf8_encode($rowProducto['nombre']); ?></td>
			        				<td><?php echo utf8_encode(mostrarProductoMarca($rowProducto['id'.DB_PREFIJO.'categoria'])); ?></td>
			        				<td><?php echo utf8_encode(mostrarNombreCategoria($rowProducto['id'.DB_PREFIJO.'categoria'])); ?></td>
			        				<td><?php echo utf8_encode($rowProducto['creado']); ?></td>
			        				<?php if($_SESSION[PREFIJO.'tipo'] == 1){ ?>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowProducto['id'.DB_PREFIJO.'producto']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowProducto['id'.DB_PREFIJO.'producto']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        				<?php } ?>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
