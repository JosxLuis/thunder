...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
    ...
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."categoria WHERE id".DB_PREFIJO."categoria= ".$_GET['id']."";
    ...
    if(isset($_POST['nombre'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."categoria WHERE 1 ".$busqueda."");
        
        if($cantidad != 0){
            $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
            mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
            $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."categoria WHERE 1 ".$busqueda." ORDER BY id".DB_PREFIJO."categoria
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."categoria WHERE 1");
        
        $cat=new pagination($cantidad,15,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."categoria  ORDER BY id".DB_PREFIJO."categoria DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
		...
		<div class="container">
			<div class="sixteen columns">
				<div class="list-items">
					<div class="titulo">
						<h4>Categorías</h4>
						<p>Categorías y subcategorías de productos</p>
					</div>
	        	...
	        	<table class="mixta">
	        		<thead>
	        			<tr>
	        				...
	        				<td width="25%">Categoría padre</td>
	        			...
	        			<?php if($cantidad != 0){ ?>
	        				<?php 
	        					while($rowCategoria = mysqli_fetch_array($resultado)){
	        						//$tipo = devolverValorQuery("SELECT tipo FROM ".DB_PREFIJO."categoria_tipo WHERE id".DB_PREFIJO."categoria_tipo =".$rowCategoria['id'.DB_PREFIJO.'categoria_tipo']" ");
	        				?>
			        			<tr>
			        				<td>#<?php echo $rowCategoria['id'.DB_PREFIJO.'categoria']; ?></td>
			        				<td><div class="foto" <?php if($rowCategoria['portada'] != "" ){ ?> style="background:url(<?php echo URL.$rowCategoria['portada']; ?>)" <?php } ?>> <?php if($rowCategoria['portada'] == "" ){ ?><i class="fa-icon-barcode"><?php } ?></div></td>
			        				<td><?php echo utf8_encode(mostrarNombreMarca($rowCategoria['id'.DB_PREFIJO.'marca'])); ?></td>
			        				<td><?php echo utf8_encode($rowCategoria['nombre']); ?></td>
			        				<td><?php echo utf8_encode(mostrarNombreCategoria($rowCategoria['parentid'])); ?></td>
			        				<td><?php echo utf8_encode($rowCategoria['creado']); ?></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowCategoria['id'.DB_PREFIJO.'categoria']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowCategoria['id'.DB_PREFIJO.'categoria']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
	        				<tr>
